...
public class GameScreen
{
    ...
    public GameScreen(Grid grid, Backdrop backdrop)
    {
        this.grid = grid;
    ...
    public void refresh(List<Input.TouchEvent> touchevents, Graphics g, Typeface font, int fontSize, int strokeWidth, int red, int green, int blue, int faintGrey, String versionNumber)
    {
        backdrop.draw(g);
        ...
        for(int i = 0; i < users.size(); i++)
        {
            ...
            if(userI.getCrash())
            {
                g.drawText("CRASH", font, 100, 100, 50, strokeWidth, Color.rgb(userI.getRed(), userI.getGreen(), userI.getBlue()));
            }
        }
    }
