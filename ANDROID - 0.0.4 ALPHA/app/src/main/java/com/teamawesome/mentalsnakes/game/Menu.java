...
public class Menu
{
    ...
    public Menu(int numButtons, int menuX, int menuY, int menuWidth, int menuHeight, int buttonWidth, int buttonHeight, Orientation orientation)
    {
        this.menuX = menuX;
        this.menuY = menuY;
    ...
    public void drawButtons(Graphics g, Typeface font, int strokeWidth, int red, int green, int blue)
    {
        ...
        {
            case VERTICAL :
                drawButtonsVertical(g, font, strokeWidth, red, green, blue);
                break;
            case HORIZONTAL :
                drawButtonsHorizontal(g, font, strokeWidth, red, green, blue);
                break;
        }
    }

    private void drawButtonsVertical(Graphics g, Typeface font, int strokeWidth, int red, int green, int blue)
    {
        for(int i = 0; i < buttons.size(); i++)
        {
            ...
            g.drawText(buttonI.getString(), font, drawX, drawY + (int)buttonHeight, 3 * (int)(buttonHeight / 4), strokeWidth, Color.rgb(220, 220, 220));
        }
    }

    private void drawButtonsHorizontal(Graphics g, Typeface font, int strokeWidth, int red, int green, int blue)
    {
        for(int i = 0; i < buttons.size(); i++)
        {
            Button buttonI = buttons.get(i);
    ...
    public void menuDoPressAction(List<Input.TouchEvent> touchevents, Grid grid, User userCurrent)
    {
        for(int i = 0; i < buttons.size(); i++)
        {
            Button buttonI = buttons.get(i);
            for(int j = 0; j < touchevents.size(); j++)
            {
                ...
                {
                    buttonI.doAction(grid, userCurrent);
                }
            }
        }
    }
